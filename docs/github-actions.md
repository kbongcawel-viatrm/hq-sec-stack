...
The dev workflow also starts the compose stack and validates the Uptime Kuma target inventory against the internal Docker endpoints defined in `The Eyes/Uptime-Kuma/monitors.yml`. Its image warmup stage prunes unused Docker system resources first, skips local build outputs and builder-stage images, attempts every remaining service pull, and fails once at the end with a consolidated list of denied or missing repositories. The nmap and Wireshark scanners keep their repo-local Dockerfiles and are excluded from the pull phase because they are build outputs, not registry pulls.
