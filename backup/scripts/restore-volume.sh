...
if [ "$#" -ne 2 ]; then
  ...
  echo "example: docker run --rm -v hq-sec-stack_graylog-data:/restore -v ./backups:/backups alpine sh /backup/scripts/restore-volume.sh graylog-data /backups/20260502T000000Z/graylog-data.tar.gz" >&2
  exit 2
fi
